...
const numberDeCasilleroLetter = document.querySelectorAll(
  ".numero-casillero-letter"
);
...
function createCasilleroNumber() {
  if (nombreInput.value != "" && apellidoInput.value != "") {
    const apellidoFirstThreeLetters = apellidoInput.value
      .toUpperCase()
      .slice(0, 3);
...
// function that removes hide class from tab content
  ...
  function validationSubmit() {
    ...
    fetch("post.php", {
      ...
      .then((data) => {
        ...
        if (data === "false") {
          alert(
            "Este correo ya existe, contáctanos para recibir la información de nuevo"
          );
        } else {
          // console.log('working')
